...
extern void client_browse_url(const char *urlbuf, char *urlbuf_varpart, const char *hoststr,void (*callback)(uint8_t,uint16_t,uint16_t),uint8_t *dstip,uint8_t *dstmac);
...
extern void client_http_post(const char *urlbuf, char *urlbuf_varpart,const char *hoststr, const char *additionalheaderline,char *postval,void (*callback)(uint8_t,uint16_t,uint16_t),uint8_t *dstip,uint8_t *dstmac);
// The callback is a reference to a function which must look like this:
// void browserresult_callback(uint8_t webstatuscode,uint16_t datapos,uint16_t len)
// webstatuscode is zero if there was no proper reply from the server (garbage message total communication failure, this is rare).
// webstatuscode is otherwise the first digit of the http status code (e.g webstatuscode=2 for 200 OK);
...
extern void client_http_put(const char *urlbuf, char *urlbuf_varpart,const char *hoststr, const char *additionalheaderline,char *postval,void (*callback)(uint8_t,uint16_t,uint16_t),uint8_t *dstip,uint8_t *dstmac);
#endif
